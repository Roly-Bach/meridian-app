...
describe('talkerPrompt — WP1 farewell dynamic-block short-circuit', () => {
  ...
  it('non-farewell wrap_up turn still gets the full dynamic block (no regression)', () => {
    const ctx = buildDynamicContext(baseContext({ phase: 'wrap_up' }))
...
describe('talkerPrompt — WP2 duplicate "Bereits erfasste Werte" removed', () => {
  it('slot_completion turn never renders the "Bereits erfasste Werte" section', () => {
    const ctx = buildDynamicContext(
      baseContext({
...
describe('talkerPrompt — WP4 extractionsLog live-feed removed', () => {
  it('never renders "Extrahierte Wissensobjekte", regardless of log content or phase', () => {
    ...
    const empty = buildDynamicContext(baseContext({ phase: 'walkthrough_step', extractionsLog: [] }))
