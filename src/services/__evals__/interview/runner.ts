...
function buildReport(opts: {
  ...
  const frontmatter = [
    ...
    `  potenzial_coverage: ${scores.potenzialCoverage}`,
    `  dedup_potenzial_coverage: ${scores.dedupPotenzialCoverage}`,
    `  dependency_capture: ${scores.dependencyCapture}`,
    `  slots_per_turn: ${scores.slotsPerTurn}`,
    `  turns_to_completion: ${scores.turnsToCompletion}`,
  ...
  const scoreTable = [
    ...
    `| potenzial_coverage | ${scores.potenzialCoverage} | maximize (KI-Potenzial: ROI-Facetten) |`,
    `| dedup_potenzial_coverage | ${scores.dedupPotenzialCoverage} | maximize (gate-Kandidat) |`,
    `| dependency_capture | ${scores.dependencyCapture} | maximize (KI-Potenzial: O6) |`,
    `| slots_per_turn | ${scores.slotsPerTurn} | maximize (Effizienz) |`,
    `| turns_to_completion | ${scores.turnsToCompletion} | — (Diagnose) |`,
...
function writeAggregateReport(opts: {
  ...
  const NUMERIC_KEYS: NumericScoreKey[] = [
    ...
    'potenzialCoverage', 'dedupPotenzialCoverage', 'dependencyCapture',
