...
describe('scorePotenzialCoverage', () => {
  ...
  it('nicht_befund_typ counts as filled', () => {
    const step = makeStep({ frequency: sv(90), error_rate_percent: svNichtBefund() })
    expect(scorePotenzialCoverage([step])).toBe(0.5)
  })

  it('all four filled → 1.0', () => {
    const step = makeStep({ frequency: sv(90), duration: sv(15), error_rate_percent: sv(5), media_breaks: sv(3) })
...
describe('scoreDedupPotenzialCoverage', () => {
  it('unions potenzial across semantically equivalent fragmented steps', () => {
    ...
    const a = makeStep({ frequency: sv(90) }, { title: 'Rechnungsprüfung' })
    const b = makeStep({ duration: sv(15) }, { title: 'Rechnungsprüfung', reihenfolge: 2 })
