...
- Requires: PROJ-25 (Prozesswissens-Schema O1–O5 + Governance) — liefert neues `StepEntry` mit O2–O5-Feldern, `potenzial`, `governance`; `MANDATORY_SLOTS` auf O1–O6 umgestellt
...
- [ ] Neuer Eval-Scorer `schemaConformanceRate` (`src/services/__evals__/interview/scorers/schemaConformanceRate.ts`):
  - Validiert alle `StepEntry`-Objekte im finalen `step_tracker` gegen eine vereinfachte Zod-Constraint-Suite (Typen, Muster, Minimalwerte aus `prozessschritt-schema.json`)
  - Score: Anteil konformer Felder je Session (0.0–1.0), Ziel ≥ 0.95
  - Erfasst Verletzungen strukturiert (welcher Step, welches Feld, welche Constraint verletzt)
- [ ] Scorer ist in `scorers/index.ts` + `runner.ts` + Langfuse-Entries integriert
- [ ] `schemaConformanceRate.test.ts`: happy path (alle Felder konform) + je 1 Verletzungsfall pro Feldtyp (TaziteSlot, TaziteSlotArray, GovernanceSlot, Potenzial-Numerik)
- [ ] Runtime-Logging: `record_slot` logt Constraint-Verletzungen per `console.warn` + Langfuse-Span-Attribut `conformanceViolation: true` — kein Hard-Error, Interview läuft weiter
- [ ] Keine neue Abhängigkeit (`ajv` oder externe JSON-Schema-Bibliothek) — Constraints in Zod implementiert, kein zusätzliches Runtime-Package
...
- **schemaConformanceRate nach PROJ-25 mit O2–O5 null**: Coverage sinkt systematisch (mehr Felder im Nenner, initial unbefüllt). Dieses Verhalten ist erwartet und muss in der Eval-Baseline dokumentiert werden — kein Scorer-Patch zur Inflation.
...
- `schemaConformanceRate`-Scorer: reine TypeScript-Logik, kein LLM-Call, importierbar aus Eval-Runner ohne Next.js-Kontext (gleiche Constraint wie `interviewSemantic.ts`)
