...
### Bugfix-Implementierung 2026-07-16 (BUG-2 + BUG-3 + BUG-5)

**BUG-2 (Off-Topic-Redirect dedupliziert nicht):** `buildOffTopicRedirect()` (`src/services/roleGuard.ts`) nahm bisher unbedingt "den letzten Assistant-Turn" als Re-Anker — war das selbst schon ein Redirect, wuchs die Nachricht bei jeder weiteren Off-Topic-Frage in Folge. Fix: die Suche überspringt jetzt Turns, die mit dem Redirect-Präfix beginnen, und verankert stattdessen auf die letzte GENUINE Frage. Zwei aufeinanderfolgende (und drei in Folge, unit-getestet) Off-Topic-Turns erzeugen jetzt dieselbe, nicht wachsende Nachricht. Neue Tests in `roleGuard.test.ts` reproduzieren Tims Turn-16→17-Muster sowie eine dreifache Kette.

**BUG-3 (stale `wrap_up`-Referenzen):**
- `timingWarning` (`talkerPrompt.ts`, `buildDynamicContext`) ersatzlos gestrichen — inkl. der KRITISCH-Variante, die de facto nie erreicht wurde (der `isCompletionFarewell`-Short-Circuit greift bereits vorher immer, sobald `timerMinutes >= maxDurationMinutes`). Keine Ersatzformulierung, wie vom Nutzer entschieden.
- Totes Schema-Feld `wrap_up_question_asked` aus `AnalystBriefingSchema` (`interviewAnalyst.ts`) und dem `AnalystBriefing`-Type (`interviewTypes.ts`) entfernt — wurde dem Analyst-LLM als Tool-Parameter angeboten, aber nirgendwo gelesen. Testfixtures in `interviewOrchestrator.test.ts`, die das Feld nur mitführten, bereinigt (3 Stellen); ein Testname entsprechend präzisiert.
- Zwei kosmetische Kommentar-Überbleibsel bereinigt: `interviewTypes.ts` (Doc-Kommentar zu `isCompletionFarewell`, "wrap_up" → "closing"), `talkerPrompt.ts:224` (Sektionskommentar `// wrap_up` → `// closing`).

**BUG-5 (keine Verabschiedung vor Clarification-Cards) — Scope-Entscheidung (Nutzer, 2026-07-16):** Talker-Call an der closing→clarification-Übergangsstelle erweitern (nicht: Verabschiedung an den Card-Submission-Endpunkt hängen). `clarification/route.ts` bleibt dadurch unverändert — der Completion-Write dort passiert bereits nach der schon gezeigten Verabschiedung.
- `talkerPrompt.ts`, `buildPhaseMethodology`'s `clarification`-Zweig (kein `hasExploringSteps`): die bisherige reine Ankündigung ("Danke! Ich habe noch ein paar kurze Abschlussfragen für dich.") ersetzt durch eine Anweisung, die eine echte Verabschiedung UND die Card-Ankündigung im selben Talker-Turn verlangt — ein LLM-Call statt zwei, entspricht der AC-Reihenfolge (Verabschiedung → Cards → completed).
- **Zusätzlicher Befund beim Umsetzen (nicht in der ursprünglichen Bug-Beschreibung):** der Eval-Runner (`src/services/__evals__/interview/runner.ts`) berechnete die Phasen-Entscheidung bisher SELBST vorab (`decideNextPhase`) und sprang bei `clarification` direkt zu `executeClarificationCompletion` — der komplette Übergangs-Turn (inkl. Talker-Call) lief für den Runner nie über `runInterviewTurn`. Damit wäre der Produktions-Fix in Eval-Transkripten nie sichtbar geworden, obwohl genau diese Transkripte den Bug ursprünglich aufgedeckt haben. Fix: der Runner lässt den Übergangs-Turn jetzt reguär über `runInterviewTurn` laufen (wie jeden anderen Turn); der Clarification-Zweig (Cards synthetisch beantworten, `executeClarificationCompletion`) greift danach, sobald `turnResult.meta.phase === 'clarification'` — mit frisch nachgeladenem `AnalystBriefing` (nach dem Background-Analyst-Pass dieses Turns), damit die synthetischen Antworten dieselben Cards sehen wie ein echter Nutzer. `preClarificationSnapshot` (L9-Scorer-Basis) wird jetzt ebenfalls erst nach diesem Übergangs-Turn geladen statt davor — präziser im Sinne des ursprünglichen Messziels ("Beitrag der Clarification-Phase", nicht des Übergangs-Turns selbst). Tote Imports (`decideNextPhase`, `OrchestratorContext`, `Phase`, `TurnMessage`) entfernt.

**Verifikation:** `tsc --noEmit` sauber. Unit-Tests: 902/903 grün (1 Skip vorbestehend) — 2 neue BUG-2-Tests (`roleGuard.test.ts`, Tims Turn-16→17-Muster + 3er-Kette) + 2 neue BUG-5-Tests (`talkerPrompt.test.ts`, Methodik-Text der clarification-Phase).

**Live-Verifikation 2026-07-16** (Eval-Preflight bestanden — Mini-Requests gegen `google/gemini-3.1-flash-lite` und `anthropic/claude-haiku-4-5` beide OK; `npm run eval:pricing-check` zeigt vollständige `MODEL_PRICING`-Abdeckung für beide Modelle):
- `npm run eval:interview -- --personas buchhalter --store pglite --runs 1 --seed 42` → `docs/evals/interview/2026-07-16/2026-07-16-18-21-08-google-gemini-3-1-flash-lite-buchhalter.md`. **Status FAIL** — erwartet, verursacht durch das weiterhin offene, bewusst zurückgestellte **BUG-1** (`dedup_slot_coverage` 0.52 < 0.75-Gate), keine Regression durch diesen Fix-Batch.
- **BUG-5 strukturell bestätigt:** letzter Agent-Turn (Turn 11) lautet "Vielen Dank für deine Zeit und die detaillierten Einblicke in deine täglichen Abläufe. Damit haben wir alle wichtigen Punkte erfasst. Bitte schaue nun auf das Interface, dort erscheinen gleich noch ein paar kurze Abschlussfragen." — echte Verabschiedung UND Card-Ankündigung in einer Nachricht, statt der vorherigen reinen Ankündigung ohne Abschied. Der Eval-Runner-Bypass-Fix griff ebenfalls: die Transition-Turn lief sichtbar durch `runInterviewTurn` (`[Agent]: ...` im stdout, im Transkript als Turn 11 protokolliert) statt stillschweigend übersprungen zu werden. Dieser konkrete Lauf hatte 0 Clarification-Cards (`[eval] Clarification phase: 0 cards`), deckt also den Verabschiedung-VOR-Cards-Fall nicht mit echten Karten ab — dafür deckt der neue `talkerPrompt.test.ts`-Unit-Test die Methodik-Anweisung direkt und lastfall-unabhängig ab. Judge-Kommentar zu `dialog_naturalness` (0.67, Gate besteht) nennt Turn 11 einen "technischen Bruch" (stilistische Randnotiz, kein Korrektheits-Befund).
- **BUG-3 bestätigt:** kein `wrap_up`-Vokabular mehr im Transkript oder in den geloggten Prompts.
- **BUG-2:** nicht im Live-Lauf getestet (Personas stellen keine Off-Topic-Fragen) — `buildOffTopicRedirect()` ist eine reine, deterministische Funktion ohne LLM-Call; die beiden neuen Unit-Tests reproduzieren Tims exakte historische Turns 16→17 plus eine 3er-Kette und decken die Korrektheit vollständig ab. Die Klassifikations-Logik (`checkRoleGuard`, LLM-Judge) wurde von diesem Fix nicht verändert und war bereits in der ursprünglichen PROJ-42-QA live verifiziert.

Status bleibt **In Review** — BUG-1 und BUG-4 weiterhin offen (zurückgestellt bis/mit PROJ-44), daher kein Übergang zu Approved durch diesen Fix-Batch. Nächster Schritt laut Sequenzierung: erneutes `/qa PROJ-42`.

### BUG-6 (NEU, gefunden bei Live-Verifikation des BUG-5-Fixes, 2026-07-16 Nachmittag): doppelte Verabschiedung

- **Severity:** Medium (kein Korrektheits-Bug — Interview terminiert weiterhin deterministisch — aber nutzersichtbar unschön, wörtliche Text-Dopplung)
- **Fundort:** `docs/evals/interview/2026-07-16/2026-07-16-18-21-08-google-gemini-3-1-flash-lite-buchhalter.md`, Turns 10–11 (Nutzer-Review des Live-Eval-Laufs aus der BUG-5-Verifikation)
- **Steps to Reproduce / Beobachtung:** Turn 10 (`phase=closing`, normaler Talker-Call, Sonde bereits in Turn 8 gestellt+beantwortet): "Vielen Dank für deine Zeit und die detaillierten Einblicke in deine täglichen Abläufe. Das Gespräch hat mir ein sehr klares Bild deiner Arbeitsprozesse vermittelt. Damit sind wir am Ende unserer Erhebung angelangt." Turn 11 (`phase=clarification`, mein heutiger BUG-5-Übergangs-Turn): "Vielen Dank für deine Zeit und die detaillierten Einblicke in deine täglichen Abläufe. Damit haben wir alle wichtigen Punkte erfasst. Bitte schaue nun auf das Interface, dort erscheinen gleich noch ein paar kurze Abschlussfragen." — wörtliche Dopplung des Einstiegssatzes.
- **Root Cause (turn-genau verifiziert über `transcript.json`):** Turn 9 landet nochmal in `explore` (Mahnwesen war zum Entscheidungszeitpunkt noch `exploring` — selbe Zustands-Verzögerung wie BUG-1, siehe unten). Dadurch ist die Phase beim Start von Turn 10 wieder `explore`, nicht `closing` — `checkLifecycle`s Trigger B prüft aber `ctx.phase === 'closing'` und greift deshalb NICHT, obwohl die Sonde längst beantwortet ist. Turn 10 läuft folglich über den normalen (nicht-`isCompletionFarewell`) `closing`-Methodik-Zweig, dessen Anweisung "Keine neuen Inhalte → kurz verabschieden" den Talker unkontrolliert eine farewell-artige Formulierung generieren lässt. Erst Turn 11 erreicht deterministisch `clarification` und trifft dort auf meinen heutigen BUG-5-Fix, der **unbedingt** eine Verabschiedung einfügt — ohne zu wissen, dass Turn 10 bereits eine lieferte.
- **Einordnung:** strukturell dieselbe Fehlerklasse wie das bereits zurückgestellte BUG-4 ("Closing-Methodik hat kein Gedächtnis"), nur an einer anderen Stelle der Closing-Methodik (dem "keine neuen Inhalte"-Zweig statt dem Sonden-Zweig) UND zusätzlich direkt durch den heutigen BUG-5-Fix verschärft (vorher endete Turn 11 nur mit der reinen Ankündigung ohne Verabschiedungswortlaut — weniger auffällig, aber ebenfalls kein sauberer Einzel-Abschied).
- **Nutzer-Entscheidung 2026-07-16:** kein isolierter schmaler Patch. Stattdessen wird die Architektur-Ursache direkt angegangen — siehe Diskussion unten.

### Diskussion 2026-07-16: löst "Analyst synchron vor Talker" (PROJ-44 Strom 5) BUG-1/BUG-4/BUG-6 automatisch?

Nutzer-Hypothese: PROJ-44 (Analyst läuft synchron VOR dem Talker statt danach/parallel, inkl. Entfernen von Quick-Extract) jetzt vorziehen (vor PROJ-43), weil das den gemeinsamen Root Cause von BUG-1/BUG-4/BUG-6 strukturell beheben könnte.

**Bestätigt (löst sich strukturell):** Der komplette Lag-Mechanismus hinter BUG-1s Symptomatik, BUG-4 und dem neuen BUG-6 entsteht dadurch, dass `decideNextPhase` für Turn N mit dem `analystBriefing`/`stepTracker`-Stand vom ENDE von Turn N-1 rechnet (voller Analyst läuft aktuell in `background()`, nach/parallel zum Talker — der KI-12-Fix `preCompletionAnalystResult` existiert nur für die `soft_confirm`-Completion-Entscheidung, NICHT für die einfache `explore→closing`-Transition). Liefe der Analyst synchron vor dem Talker, sähe die Phasenentscheidung für Turn N den Inhalt von Turn N selbst — das eliminiert die Ein-Turn-Verzögerung, die alle drei Symptome (Monatsabschluss-Themensprung ohne Überleitung, Mahnwesen-Rückkehr nach explore, doppelte Verabschiedung) strukturell verursacht.

**NICHT automatisch gelöst (zwei eigenständige Lücken, dokumentiert 2026-07-16, noch unverifiziert):**
1. **BUG-1s Kern-Kalibrierung** (`dedup_slot_coverage` 0.52 im aktuellen Live-Lauf, Gate ≥0.75): laut Hypothese in der Spec zwei Ursachen — Staleness UND dass `step_advance_ready` laut Analyst-Prompt zu großzügig "true" setzen darf ("auch wenn nicht jeder optionale Slot gefüllt ist"). Sync-Timing behebt nur die Staleness; die Advance-Schwelle selbst bliebe unverändert großzügig — Explore könnte weiterhin zu früh enden, nur mit aktuelleren statt veralteten Daten als Grundlage.
2. **Clarification-Cards-Zuverlässigkeit:** BUG-1-Hypothese notierte bereits, dass das Cards-Sicherheitsnetz trotz PFLICHT-Anweisung im Analyst-Prompt ("durchsuche ALLE registrierten Schritte auf null-Pflicht-Slots") nicht zuverlässig greift — im heutigen Live-Lauf z.B. blieb `Mahnwesen.duration_minutes` null, ohne dass eine Card dafür generiert wurde (0 Cards insgesamt). Das ist ein Prompt-Adherence-Problem im Card-Erzeugungs-Pfad, unabhängig von sync/async-Timing.

**Entscheidung (Nutzer, 2026-07-16):** `/write-spec PROJ-44` + `/grilling` als nächster Schritt, VOR PROJ-43 (Umkehr der ursprünglich am 2026-07-15 festgelegten Reihenfolge "PROJ-42+43 vor Strom 5+6"). Der schmale BUG-6-Einzelpatch entfällt dadurch bewusst — würde durch die PROJ-44-Umstellung ohnehin überschrieben. PROJ-42 bleibt bis zur PROJ-44-Umsetzung **In Review** mit drei offenen, zusammenhängenden Punkten (BUG-1, BUG-4, BUG-6), die gemeinsam nach PROJ-44 erneut bewertet werden — kein separates `/qa PROJ-42` davor, da BUG-2/3/5 bereits einzeln live verifiziert sind und der Rest strukturell an PROJ-44 hängt.
