...
| PROJ-36 | ProcessStepsTable — Cluster-Aggregation als reines Modul | Revision | Dashboard & Output | PROJ-20 | Approved | [spec](dashboard-output/PROJ-36-process-steps-table-cluster-aggregation.md) | P2 | S | 0:0:0 |
...
| KI-13 | 🔴 Open (Deploy-Gate) | E2E-Suite (`npm run test:e2e`) zeigt 22 Fails, alle vorbestehend und unrelated zu PROJ-36 verifiziert (gleicher Fail auf Pre-PROJ-36-Commit `89ce1bf`): **10x** CSP-Nonce-Hydration-Mismatch (`nonce="<uuid>"` Server vs. `nonce=""` Client) in `tests/PROJ-23-adaptive-clarification-questions.spec.ts`, chromium + Mobile Safari — Dev-Mode-spezifisch, vermutlich PROJ-15-Bereich (`next.config.ts` CSP-Header). **12x** Mobile-Safari `waitForURL`-Timeouts beim Login (`PROJ-3`, `PROJ-6`) — bei Retry als flaky bestätigt (gleiche Nonce-Warnung im WebServer-Log), keine deterministischen Fails. Verstößt gegen general.md Hard Rule „100% Tests grün vor Deploy" — PROJ-36 wurde mit dokumentierter Ausnahme deployed (0 eigene Bugs, Diff berührt keinen der betroffenen Pfade: kein CSP/Auth/Login-Code). Vor dem **nächsten** Feature-Deploy verbindlich fixen, nicht weiter mitschleppen. | E2E-Suite-Lauf 2026-06-25 (/deploy PROJ-36) | M (CSP-Nonce-Root-Cause + Webkit-Timing) |
