...
| KI-12 | 🟡 Fix implementiert, Live-Verifikation ausstehend | ~~`dedup_slot_coverage`-Gate (`≥ 0.75`) ist der dominante reale FAIL-Grund über die Buchhalter-Lauf-Historie (10 Läufe 2026-06-22/23: 6 PASS / 3 FAIL / 1 PARTIAL). Wiederkehrendes Muster: dritter Prozess (meist spät entdeckt, z.B. Mahnlauf) bleibt bei `walkthrough` statt `done`.~~ → Root Cause 2026-06-24 gefunden: [interviewOrchestrator.ts](../src/services/interviewOrchestrator.ts) `checkLifecycle()` entscheidet `soft_confirm` anhand des `stepTracker`-Snapshots vom START des Turns — der Analyst läuft im Dual-Loop (ADR-011) im Hintergrund und hat den GERADE eingegangenen `userInput` noch nicht gesehen. Antwortet der Mitarbeiter auf die Wrap-up-Frage "haben wir etwas übersehen?" mit einem neuen Prozess, entscheidet die Lifecycle-Prüfung auf Basis des alten (2-Prozesse-)Trackers → Interview endet, bevor der neue Prozess je befragt wird. Fix: [runInterviewTurn.ts](../src/services/runInterviewTurn.ts) — bei `reason==='soft_confirm'` läuft `runAnalystOnline` jetzt synchron VOR der Completion-Entscheidung, Tracker wird neu geladen, `checkLifecycle` erneut aufgerufen; findet sich ein neu registrierter Schritt, wird die Completion verworfen und der Turn normal fortgesetzt (kein Doppel-Run — Hintergrund-Closures geben das bereits berechnete Ergebnis zurück). 1 neuer Regressionstest (T11) reproduziert exakt den Mahnwesen-Fall, 11/11 `runInterviewTurn`-Tests grün, 681 Tests gesamt grün. **Offen:** kein Live-Eval-Lauf — nächster Schritt laut neuem Multi-Run-Workflow (eval-interview/SKILL.md): `--runs 3` vor/nach Vergleich, nicht Einzellauf-Anekdote. | Eval 2026-06-22, 2026-06-23 (über 10-Lauf-Historie identifiziert) | Fix implementiert 2026-06-24, M (Live-Verifikation mit `--runs 3` ausstehend) |
