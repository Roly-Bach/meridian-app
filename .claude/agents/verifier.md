...
7. **Spec-Coverage-Check**: Lese die Feature-Spec (Pfad aus `features/INDEX.md`, z.B. `features/platform/PROJ-X-*.md`). Prüfe für jede Acceptance Criterion: existiert mindestens ein Test (Unit oder E2E), der sie abdeckt? Fehlende Abdeckung → `spec_coverage: fail`.
